...
void LinkedCellContainer<2>::linkCells() {
  ...
  for (int x = 0; x < cellsPerRow; ++x) {
    for (int y = 0; y < cellsPerColumn; ++y) {
      auto index = static_cast<size_t>(getIndexBasedOnCoordinates({static_cast<double>(x * cellSize[0]), static_cast<double>(y * cellSize[1])}));
      ...
      for (int nX = x - 1; nX <= x + 1; ++nX) {
        for (int nY = y - 1; nY <= y + 1; ++nY) {
          ...
          // get neighbours
          auto posNeighIndex =
              static_cast<size_t>(getIndexBasedOnCoordinates({static_cast<double>(nX * cellSize[0]), static_cast<double>(nY * cellSize[1])}));
